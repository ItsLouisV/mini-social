...
/// Web IndexedDB Cache Engine using Hive
class HiveWebService {
  Box? _messagesBox;
  ...
  static Future<HiveWebService?> init() async {
    if (!kIsWeb) return null;

    try {
      await Hive.initFlutter();
      final service = HiveWebService();
      service._messagesBox = await Hive.openBox('viora_web_messages');
      service._conversationsBox = await Hive.openBox('viora_web_conversations');
      service._postsBox = await Hive.openBox('viora_web_posts');
      service._profilesBox = await Hive.openBox('viora_web_profiles');
      service._notificationsBox = await Hive.openBox('viora_web_notifications');
      service._searchHistoryBox = await Hive.openBox('viora_web_search_history');
      service._syncQueueBox = await Hive.openBox('viora_web_sync_queue');

      debugPrint('🌐⚡ [HiveWebService] Web IndexedDB Cache initialized successfully via Hive');
      return service;
    } catch (e) {
      debugPrint('⚠️ [HiveWebService] Failed to initialize Web IndexedDB: $e');
      return null;
    }
  }
  ...
  List<Map<String, dynamic>> getConversations() {
    ...
    items.sort((a, b) {
      final aTime = DateTime.tryParse(a['last_message_at']?.toString() ?? '') ?? DateTime(1970);
      final bTime = DateTime.tryParse(b['last_message_at']?.toString() ?? '') ?? DateTime(1970);
      return bTime.compareTo(aTime);
    });
    return items;
  }
  ...
  Future<void> saveSearchQuery(String query) async {
    ...
    await _searchHistoryBox!.put(clean.toLowerCase(), jsonEncode({
      'query': clean,
  ...
  // ── Sync Queue (Outbox) ──
  Future<void> enqueueSyncAction(String id, String actionType, Map<String, dynamic> payload) async {
    if (_syncQueueBox == null) return;
    await _syncQueueBox!.put(id, jsonEncode({
      'id': id,
